...
class ThemeWidget(QWizardPage):
    def __init__(self, parent=None):
        ...
        label = QLabel(self)
        label.setText(self.tr("<p>Choose your favorite theme and desktop type. Customize LupuS with colorful styles and themes.</p>"))
    ...
    def iconSetSelect(self, item):
        self.iconSet = getattr(item, "iconThemeName", str(item.text()).lower())

    def mouseCursorSelect(self, item):
        self.mouseCursor = getattr(item, "cursorThemeName", item.text())
    ...
    def execute(self):
        ...
        if self.colorScheme != None:
            if os.path.isabs(self.colorScheme):
                color_path = self.colorScheme
            else:
                color_path = join("/usr/share/color-schemes", self.colorScheme)
            colorSettings = QSettings(color_path, QSettings.Format.IniFormat)

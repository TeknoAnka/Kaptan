...
class SummaryWidget(QWizardPage):
    ...
    def summaryWrite(self):
        ...
        color_scheme_display = self.tr("Unspecified.")
        if themeWidget.colorScheme:
            color_scheme_display = os.path.basename(themeWidget.colorScheme).split(".")[0].replace("-", " ").title()
        ...
        self.labelSummary.setText(html.format(mouseButtonMap, folderSingleClick, themeWidget.desktopCount, desktopType,
                          ...
                          (themeWidget.iconSet or self.tr("Unspecified.")).capitalize()))
